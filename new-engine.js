...
function Sb() {
    ub ? Tb() : (ub = true, Ub(), m.push(new Vb("player")), m.push(new Vb("opponent")), $(window).resize(Wb), Wb(), $("#game-navbar").remove(), $(window.document).contextmenu(function() {
            Xb();
            return false
        ...
        wa = $("#game-selection-card-template").remove().removeAttr("id"), xa = $("#game-announce-template").remove().removeAttr("id"), ya = $("#game-announce-card-template").remove().removeAttr("id"), $("#game-action-menu").mouseleave(function() {
            ...
            ac();
            fc(m[D.controller].c[D.location], true);
            vb = true
            ...
            P(Ja ? -1 : 0);
            N()
        }), $("#game-announce-card-text").on("input", sc), $("#game-timer-bar-player").hide(), $("#game-timer-player").hide(), $("#game-timer-bar-opponent").hide(), $("#game-timer-opponent").hide(), wb = new Audio(h("musics/14-seal-of-orichalcos.mp3")), wb.loop = true, tc("music"), $("#game-field").on("dragstart", function() {
            return false
...
function lc(a) {
    ...
    1 == b.length ? (M(p === L.J ? Tc : Uc, v.indexOf(a)), N()) : 1 < b.length && (Na = true, Vc(b, true, true))
}
...
function Ac() {
    if (t) {
        ...
        a = Rc();
        if (a >= cb && a <= db) return Ja ? Sc(y[0]) : (Xc(), N()), true
    }
    return false
}
...
function Sc(a) {
    if (Ja) {
        ...
        1 === b.length ? (P(x.indexOf(a)), N()) : 1 < b.length && (Na = true, Vc(b, true, true))
...
function cd() {
    ...
    hd("music", 0, 100, 20);
    hd("speed", 20, 300, 100);
    id("auto-place-monsters", false);
    id("auto-place-spells", true);
    f.save();
    tc("sounds");
    tc("speed");
    jd = tc;
...
function od() {
    ...
    G.onclose = function() {
        G = null;
        Ta && (Ta = false, N(), pd("Error!", "You have been disconnected from the server."), $("#game-chat-textbox").remove())
    };
    G.onerror = function() {
...
function Ed() {
    Jb ? ($("#game-rps-rock").show(), $("#game-rps-paper").show(), $("#game-rps-scissors").show()) : (Jb = true, $("#game-rps-container").show(), $("#game-rps-first").hide(), $("#game-rps-second").hide(), $("#game-rps-rock").click(function() {
        Qe();
        Re(2)
...
function Fd(a) {
    Jb || (Jb = true, $("#game-rps-container").show(), $("#game-rps-first").hide(), $("#game-rps-second").hide());
    Qe();
...
function Te() {
    ...
    Ue("side");
    $c();
    Mb || (Mb = true, $(window).on("mouseup", function(a) {
        if (1 === a.which && J) return Ve(), false
    }))
}
...
function Pd(a) {
    if (p !== a.phase) return -1 !== p && n[p] && n[p].removeClass("engine-button-success").addClass("engine-button-default"), p = a.phase, n[p] && n[p].addClass("engine-button-default").addClass("engine-button-success"), Q("next-phase"), (a = ff[p]) && ef(a), true
}
...
function gf(a) {
    if (0 === a.previousLocation) {
        ...
        true), S(b, a.currentController, a.currentLocation, a.currentSequence, true), df(b, a.cardCode, a.currentPosition, 300 * C, W))
}
...
function kd(a, b) {
    Ob || (Ob = true, $("#game-alert-title").text(a), $("#game-alert-text").html(b), $("#game-alert-window").show(), $("#game-alert-darkener").show())
}

function md() {
    Ob && (Ob = false, $("#game-alert-window").hide(), $("#game-alert-darkener").hide())
}
...
function ue(a) {
    return yf(a, false, false)
}

function xe(a) {
    return yf(a, true, false)
}

function ve(a) {
    return yf(a, false, true)
}
...
function yf(a, b, c) {
    ...
    if (c) {
        ...
        d ? fc(c, false) : Af(c, b)
    } else d ? fc(a.cards, false) : Af(a.cards, b)
}
...
function we(a) {
    a.isChain && 1 !== mb ? P(-1) : (R(), Fa = t = true, db = cb = a.cards.length, Da = true, fc(a.cards, false))
}
    ...
    if (a.forced || 2 !== mb)
        if (0 !== mb || a.forced || 0 !== a.cards.length && 0 !== a.specialCount)
            if (a.forced && 1 === a.cards.length) P(0);
                ...
                a.forced ? rc() : (Fa = true, $("#game-yesno-title").text("Activate a card? " +
...
function Ge(a) {
    ...
    Df(a.availableAttributes, false)
}
...
function R() {
    return vb ? (vb = Ma = false, x = [], $("#game-selection-list").empty(), $("#game-selection-window").hide(), Nc(), true) : false
}
...
function N() {
    ...
    Nc();
    vb = Na = Oa = Ma = La = Ka = Ja = Fa = Ga = Da = Ea = Ca = t = false;
...
var Z = {
    ...
    Fb: function() {
        ...
        $(window).on("mouseup", function(a) {
            if (1 === a.which && Z.selection) return Z.aa(), false
        });
        $(window).mousemove(function(a) {
            Z.Ib = a.pageX;
            Z.Jb = a.pageY;
...
function Dc(a, b) {
    ...
    if (c.b !== b && (c.b = b, null !== b))
        if ((c = X[b.code]) ? a.find(".card-name").text(c.name) : a.find(".card-name").text("id: " + b.code), b.type & U.L ? (a.find(".card-if-monster").show(), a.find(".card-race").text(Ef[b.race]), a.find(".card-attribute").text(Ff[b.H]), a.find(".card-attack").text(b.attack), a.find(".card-defence").text(b.type & U.C ? "LINK-" + b.V : b.i), a.find(".card-level").html(b.type & U.G ? b.W : b.level)) : a.find(".card-if-monster").hide(), b.type & U.C ? a.find(".card-if-not-link").hide() : a.find(".card-if-not-link").show(),
            ...
            var d = true,
                e;
            for (e in b.F) d || (c += "<br>"), d = false, c += b.F[e] + " \u00d7 " + xf[e];
